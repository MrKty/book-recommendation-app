...
const BookInfoTabs: FC<BookInfoTabsProps> = ({ book }) => {
  ...
  const review = useSelector((state: RootState) => state.reviews[book.id] || { rating: 0, text: '' });
  ...
  return (
    <Tabs defaultActiveKey="info" tabBarStyle={{ marginBottom: 24 }} className="custom-tabs">
      <Tabs.TabPane tab="Book Info" key="info">
        ...
        <Collapse bordered={false} style={{ marginTop: 24 }} expandIconPosition="end" defaultActiveKey={[]}>
          <Collapse.Panel header={<Text strong>Book details</Text>} key="1" style={{ background: 'transparent' }}>
            <Descriptions column={2} size="small" labelStyle={{ width: 150 }} style={{ maxWidth: 600 }}>
